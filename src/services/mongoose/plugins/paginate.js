...
export default function paginatePlugin(schema, options) {
	schema.statics.paginate = async function(query, select, cursor, view) {
		...
		const [count, rows] = await Promise.all([this.countDocuments(query), this.find(query, select, cursor)])
		...
		return { rows: view ? rows.map(row => row.view()) : rows, count, nextPage, prevPage, page }
	}
}
